...
class TestCreateOrders:
    ...
    def test_create_order_with_auth(self, setup_and_teardown):
        ...
        response = self.client.post(API_ENDPOINTS["create_order"], Ingredients.correct_ingredients_data, token)
    ...
    def test_create_order_without_auth(self, setup_and_teardown):
        response = self.client.post(API_ENDPOINTS["create_order"], Ingredients.correct_ingredients_data)
        assert response.status_code == 200 and response.json().get("success") is True

    @allure.title("Создание заказа без ингредиентов")
    def test_create_order_without_ingredient(self, setup_and_teardown):
        response = self.client.post(API_ENDPOINTS["create_order"])
    ...
    def test_create_order_incorrect_ingredient(self, setup_and_teardown):
        response = self.client.post(API_ENDPOINTS["create_order"], Ingredients.incorrect_ingredients_data)
